...
class NoLeaf[A:ClassManifest](val center:Vec3i, val radius:Int, val v:A) extends Node[A]{
	...
	def update(pos:Vec3i,v:A) = {
		val index = flatIndex(pos)
		data(index) = data(index).update(pos,v)
...
class Octree[A:ClassManifest](logOfSize:Int,init:A) extends Data3D[A] with Serializable with Iterable[NodeInfo[A]]{
	...
	def iterator = new Iterator[NodeInfo[A]]{
		...
		case n:Leaf[A] => 
			...
			NodeInfo(n.center-n.radius,n.radius*2,n.v)
		case n:NoLeaf[A] => 
...
/*
class WorldOctree(logOfSize:Int) extends Octree[Hexaeder](logOfSize:Int,null){
	def genvertexarray = {

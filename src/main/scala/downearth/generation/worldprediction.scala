...
object prediction {
  ...
  def apply(world:Interval3) = {
    val trivialBounds = trivial(world)
  ...
  def trivial(world:Interval3) = {
    ...
    def perlinnoise3_uid4e7b7b1d(v:Interval3, x:Interval, y:Interval, z:Interval, add:Interval, sub:Interval, size:Double, scale:Double, offset:Double):Interval = {
      (noise.perlin.prediction.trivialSimple((v + Interval3(x,y,z))*size)+offset)*scale/size + add - sub}
